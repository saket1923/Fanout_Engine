 ...
 * Mock gRPC sink.
 *
 * - Transforms {@link Record} to a Protobuf-like mock string using {@link GrpcTransformer}
 * - Simulates streaming by printing the payload
 * - Enforces a simple rate limit (records/sec) via {@link Thread#sleep(long)}
 * - Simulates ~10% failures and retries up to 3 attempts
 */
public class GrpcSink implements Sink<String> {

  private static final String SINK_TYPE = "GRPC";
  ...
  public GrpcSink(ConfigLoader configLoader) {
  ...
  public void send(String payload) {
    ...
    for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      if (simulateFailure()) {
        System.err.println("GrpcSink: stream send failed (attempt " + attempt + "/" + MAX_ATTEMPTS + ")");
      ...
      System.out.println("GrpcSink: stream payload=" + payload);
    ...
    if (!sent) {
      System.err.println("GrpcSink: giving up after " + MAX_ATTEMPTS
